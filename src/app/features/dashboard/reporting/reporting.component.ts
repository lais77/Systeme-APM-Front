...
export class ReportingComponent implements OnInit, AfterViewInit {
  ...
  displayReportRows: any[] = [];
  displayReportTotals = { totalPlans: 0, totalActions: 0, actionsCloturees: 0, tauxGlobal: 100 };
  ...
  chargerStats(): void {
    ...
    forkJoin({
      dept: this.http.get<any[]>(API.stats.byDepartment),
      ...
      error: (err) => {
        console.error('Reporting stats error:', err);
        this.chargement = false;
        this.updateDerivedStats();
        setTimeout(() => this.creerGraphiqueEvolution(), 100);
      }
  ...
  private updateDerivedStats(): void {
    if (!this.statsDepartements.length) {
      this.displayReportRows = this.fallbackRows;
    } else {
      this.displayReportRows = this.statsDepartements.map((dept: any) => {
    ...
    const rows = this.displayReportRows;
    ...
    this.displayReportTotals = { totalPlans, totalActions, actionsCloturees, tauxGlobal };
  }

  get reportRows() {
    return this.displayReportRows;
  }

  get reportTotals() {
    return this.displayReportTotals;
  }
